"""v1 SQLite persistence - LEGACY, retired in M9-T1.

This is the v1 store, moved here unchanged when M3-T1 rewrote `store.py` for the
v2 schema. It exists only so the v1 modules still in the tree (runner, output,
serve, review, cursor, llm) and their tests keep importing until M9 retires them.

It refuses to open a v2 database: both default to data/jobscraper.db, and
`CREATE TABLE IF NOT EXISTS` would otherwise quietly bolt v1 tables onto the v2
schema - the kind of corruption nobody notices until a query returns nonsense.
...
class Store:
    def __init__(self, path: Path, check_same_thread: bool = True):
        ...
        if "key" in cols:
            self.conn.close()
            raise RuntimeError(
                f"{path} is a v2 database; this is a retired v1 command. "
                "Use the v2 commands (see the PRD, section 0.6).")
